				...
				public void run() {
					...
					if (args[0].equalsIgnoreCase("help")) {
						...
						if (args.length == 1 || (args.length >= 2 && args[1].equalsIgnoreCase("help"))) {
							if (player == null || player.hasPermission("skyblockearth.admin")
		...
		if (command.getName().equalsIgnoreCase("island")) {
			...
			if (args.length == 1) {
				if (args[0] == null || args[0].isEmpty()) {
					...
					if (sender.hasPermission("skyblockearth.admin") || sender.hasPermission("skyblockearth.admin.*")
							|| sender.hasPermission("skyblockearth.*")) {
